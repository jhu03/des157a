(function () {
	...
	function throwDice() {
		...
		// 	actionArea.innerHTML += `<p>Sorry, one of your rolls was a one, switching to ${
		...
		// if neither die is a 1...
			...
			document.getElementById("pass").addEventListener("click", function () {
				...
				pass.play();
				gameData.index ? (gameData.index = 0) : (gameData.index = 1);
				gameData.rollSum = 0; //glenda suggestion
				setUpTurn();
			});
	...
	function blockGen() {
		...
		for (let i = 0; i < gameData.rollSum; i++) {
			...
			if (gameData.numBlocks[gameData.index] === 0) {
				console.log("Bottom Block", gameData.numBlocks[gameData.index]);
				blocks.innerHTML += `<div id="block${
					gameData.numBlocks[gameData.index]
				}" class="blocks appear"><img src=${blockImg[0]}></div>`;
			} else {
				console.log("Style for player 2");

				blocks.innerHTML += `<div id="block${
					gameData.numBlocks[gameData.index]
				}" class="blocks appear"><img src="${
					blockImg[randomNum(1, blockImg.length - 1)]
				}" width="168" height="88" alt="tower block"></div>`;

				// random location of block placement
				document.getElementById(
					`block${gameData.numBlocks[gameData.index]}`
			...
			console.log(
				`Player ${gameData.index + 1} numBlock ${
					gameData.numBlocks[gameData.index]
				}`
			);
		}

		// const reduceHeight = -88 * `${gameData.numBlock[gameData.index]}`;
	...
	function backgroundMove() {
		...
		if (gameData.numBlocks[gameData.index] % 6) {
			backgroundImage.style.backgroundPositionY = `${position * 80}px`;
			//Don't use percentages
			blocks.style.transform = `translateY(${
				gameData.numBlocks[gameData.index] % 6
			} * 352px)`;
